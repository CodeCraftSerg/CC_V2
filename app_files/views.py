...
def main(request):
    ...
    """
    return render(
        request, "app_files/files_page.html", context={"title": "Download files"}
    )
...
def edit_description(request, f_id):
    if request.method == "POST":
        ...
        UserFile.objects.filter(pk=f_id, user=request.user).update(
